...
int retornaNombreConfeById(eConfederaciones confederacion[], int size, int idBuscado, char* nombreConfederacion)
{
	int retorno;
	if(confederacion != NULL && size > 0 && idBuscado>0)
	{
		for(int i=0; i<size; i++)
		{
			if(confederacion[i].id == idBuscado)
			{
				strncpy(nombreConfederacion,confederacion[i].nombre,LARGO);
				retorno=0;
				break;
			}
		}
